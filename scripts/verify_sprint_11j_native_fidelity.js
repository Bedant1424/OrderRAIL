...
const OUTPUT_DIR = path.join(ROOT_DIR, 'sprint_11j_screenshots');

if (!fs.existsSync(OUTPUT_DIR)) {
  fs.mkdirSync(OUTPUT_DIR, { recursive: true });
}

async function verifySprint11J() {
  ...
  for (const tNum of tablesToTest) {
    const dataUrl = await page.evaluate(async (num) => {
      ...
      qrCtx.fillRect(0, 0, 360, 360);
      qrCtx.fillStyle = "#321300";
      qrCtx.fillRect(20, 20, 100, 100);
      qrCtx.fillRect(240, 20, 100, 100);
      qrCtx.fillRect(20, 240, 100, 100);
      ...
      return new Promise((resolve, reject) => {
        templateImg.onload = () => {
          ...
          // 2. Render ONLY numeric value centered inside white rounded rectangle
          ...
          ctx.fillText(numericOnly, 511, 445);

          // 3. Draw White Backing Card
          const cardX = (1023 - 420) / 2;
          const cardY = 640;
          const cardSize = 420;
          const r = 28;
          ...
          // 4. Draw Dynamic QR Code
          ctx.drawImage(qrCanvas, (1023 - 360) / 2, 670, 360, 360);
    ...
    const outPath = path.join(OUTPUT_DIR, `native_table_${tNum}_artwork.png`);
  ...
  console.log("SPRINT_11J_VERIFICATION_COMPLETE");
  console.log(JSON.stringify(generatedArtworks, null, 2));

  await browser.close();
}

verifySprint11J().catch(console.error);
