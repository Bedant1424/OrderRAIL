...
const OUTPUT_DIR = path.join(ROOT_DIR, 'sprint_11m_screenshots');

if (!fs.existsSync(OUTPUT_DIR)) {
  fs.mkdirSync(OUTPUT_DIR, { recursive: true });
}

async function verifySprint11M() {
  ...
  for (const tNum of tablesToTest) {
    const dataUrl = await page.evaluate(async (num) => {
      ...
      qrCtx.fillRect(210, 20, 90, 90);
      qrCtx.fillRect(20, 210, 90, 90);
      ...
      return new Promise((resolve, reject) => {
        templateImg.onload = () => {
          ...
          // 2. Draw White QR Backing Card (Enlarged to 390px, ~90% fill of cream container)
          const cardX = (1023 - 390) / 2;
          const cardY = 615;
          const cardSize = 390;
          const r = 20;
          ...
          // 3. Draw Dynamic QR Code (Scaled proportionally to 320px)
          ctx.drawImage(qrCanvas, (1023 - 320) / 2, 650, 320, 320);

          // 4. Render Numeric Table Number
    ...
    const outPath = path.join(OUTPUT_DIR, `fit_table_${tNum}_artwork.png`);
  ...
  console.log("SPRINT_11M_VERIFICATION_COMPLETE");
  console.log(JSON.stringify(generatedArtworks, null, 2));

  await browser.close();
}

verifySprint11M().catch(console.error);
