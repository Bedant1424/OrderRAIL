...
export interface ModernOccupiedTablesWidgetProps {
  tables: TableRow[];
  orders: Order[];
...
export default function ModernOccupiedTablesWidget({
  tables,
  orders,
  onSelectTable,
  selectedTableId = null,
}: ModernOccupiedTablesWidgetProps) {
    ...
    <div className="rounded-3xl bg-card p-4 sm:p-5 shadow-soft ring-1 ring-border/60 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-display text-sm font-bold text-foreground">Restaurant Floor</h3>
            <p className="text-[11px] text-muted-foreground">
              {occupiedCount} of {totalTables} tables active · Click table for order details
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2 pt-1">
        {sortedTables.map((table) => {
          ...
          const chipBgMap = {
            green: "bg-secondary/40 text-foreground hover:bg-secondary border-border/60",
            ...
            blue: "bg-blue-500/10 text-blue-900 border-blue-500/30 dark:text-blue-200 hover:bg-blue-500/20 font-bold",
          ...
          return (
            ...
            >
              ...
              {statusInfo.activeOrders.length > 1 && (
