...
import { Search, ShoppingBag, Plus, Minus, Trash2, Send, Utensils, Sparkles, AlertTriangle, ArrowRight } from "lucide-react";
import { cn } from "@/lib/utils";
import { toast } from "sonner";
import { BillingDrawerV3 } from "./BillingDrawerV3";
...
export function OrderWorkspaceV3() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "F2") {
        ...
        e.preventDefault();
        handleSubmitKot();
      } else if (e.key === "F8" || e.key === "F10" || e.key === "F11") {
        e.preventDefault();
  ...
  const sessionMetadata = selectedTable
    ? `${selectedTable.label} (${selectedTable.status} • ${selectedTable.seats} Seats)`
    : "Table 4 (Dine-In • 4 Guests • 34m ago)";
  ...
  const handleAddItem = (item: CatalogItem) => {
    setCart((prev) => {
      ...
      return [...prev, { id: `c-${Date.now()}`, name: item.name, price: item.price, qty: 1 }];
    });
    toast.success(`Added ${item.name} to cart`);
  };

  const handleSubmitKot = () => {
    if (cart.length === 0) {
      toast.error("Cart is empty!");
      return;
    }
    toast.success(`✅ KOT Ticket #104 Submitted to Kitchen (${activeLabel})`);
  };
  ...
  return (
    <div className="flex flex-col h-full bg-card rounded-3xl p-4 shadow-soft ring-1 ring-border/60 justify-between select-none">
        ...
        <div className="flex items-center justify-between border-b border-border/60 pb-2">
          ...
          <div className="flex items-center gap-1.5 p-1 rounded-2xl bg-secondary/80 text-xs font-semibold">
            <button
              ...
              className={cn(
                "px-2.5 py-1 rounded-xl transition flex items-center gap-1",
                mode === "dine-in"
      ...
      <div className="pt-3 border-t border-border/60 flex flex-col gap-2">
        ...
        <div className="grid grid-cols-3 gap-2 mt-1">
          ...
          <button
            ...
            className="rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white py-2.5 text-xs font-bold shadow-soft transition flex items-center justify-center gap-1 active:scale-95"
          >
      ...
      {/* Contextual Billing Slide-Over Drawer */}
      <BillingDrawerV3
